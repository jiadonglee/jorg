#!/usr/bin/env python3
"""
Final test of Jorg EOS module functionality and comparison with Korg.
"""
...
def test_gas_pressure(korg_data):
    ...
    if errors:
        print(f"  Found {len(errors)} cases with large errors:")
        for T, n, korg_p, jorg_p, err in errors[:5]:  # Show first 5
            print(f"    T={T}, n={n:.2e}, Korg={korg_p:.6e}, Jorg={jorg_p:.6e}, rel_err={err:.2e}")
...
def test_electron_pressure(korg_data):
    ...
    if errors:
        ...
        for T, ne, korg_p, jorg_p, err in errors[:5]:  # Show first 5
            print(f"    T={T}, ne={ne:.2e}, Korg={korg_p:.6e}, Jorg={jorg_p:.6e}, rel_err={err:.2e}")
...
def test_density_from_pressure(korg_data):
    ...
    if errors:
        ...
        for T, P, korg_n, jorg_n, err in errors[:5]:  # Show first 5
            print(f"    T={T}, P={P:.2e}, Korg={korg_n:.6e}, Jorg={jorg_n:.6e}, rel_err={err:.2e}")
...
def test_constants():
    ...
    # Check if our Boltzmann constant matches approximately
    print(f"  Jorg k_boltzmann_cgs = {k_boltzmann_cgs:.10e}")
    ...
    print(f"  Test calculation: P = n*k*T")
    print(f"    n = {n:.2e} cm^-3")
    print(f"    T = {T} K")
    print(f"    P = {P:.6e} dyne cm^-2")

"""Helium continuum absorption implementations."""
...
def he_minus_ff_absorption(
    ...
    if np.isscalar(wavelength_A):
        return float(np.asarray(K_he_proper).reshape(-1)[0] * n_he_i_ground * P_e)
...
def _helium_free_free_john1994(wavelength_A: float, theta: float) -> float:
    ...
    wl_arr = np.asarray(wavelength_A, dtype=np.float64)
    th_arr = np.asarray(theta, dtype=np.float64)
    wl_arr, th_arr = np.broadcast_arrays(wl_arr, th_arr)

    # Korg behavior for out-of-grid points is zero.
    ...
    k_interp = np.asarray(k_interp, dtype=np.float64)
